...
### [StringToTypeConverter][string-to-type-converter-url]

[`StringToTypeConverter<Object>`][string-to-type-converter-url] performs conversion from string to typed instances and is used when querying registry entities.
...
### [StringToEntityPropertyMapConverter][string-to-entity-property-map-converter-url]

[`StringToEntityPropertyMapConverter`][string-to-entity-property-map-converter-url] is also used for querying registry entities. It is responsible for creating conditions from query string and a list
of properties to search (conversion to typed instances is delegated to [`StringToTypeConverter<Object>`][string-to-type-converter-url]).
When searching using [`StringSearchExecutor`][search-executor-url] it accepts a query string, and a list of properties to be searched that are then converted to properties using
[`StringToEntityPropertyMapConverter`][string-to-entity-property-map-converter-url].

---

### [RepositoryFactorySupportFactory][repository-factory-support-factory-url]
...
- [`NaturalIdSearchExecutor<T>`][natural-id-search-executor-url].

### Using [`SearchExecutor`][search-executor-url] (standard search)
...
To be able to use [`SearchExecutor`][search-executor-url] user-defined JPA repository has to extend [`SearchExecutor`][search-executor-url] interface.
...
In this service we have defined that the query will be executed on the entity `Car` based on the `SearchCarRequest` class and that results of the search will be of type `CarSearchResult`.
...
From and To suffixes represent range search so `manufacturedDateFrom` searches `manufacturedDate` property on Car that is greater than the one in `SearchCarRequest`.
The same applies to `priceFromIncluding` and `priceTo`, only difference that starting price is included.
...
Therefore, resulting query for `SearchCarRequest` request whose values would be :
...
### Using [`StringSearchExecutor`][string-search-executor-url] (quick search)
...
The [`StringSearchExecutor`][string-search-executor-url] query class is different from the [`SearchExecutor`][search-executor-url] class in sense that it contains a search term and a property list.
...
We can reuse the same result class `SearchCarResult` that was demonstrated in the [`SearchExecutor`][search-executor-url] usage demonstration.
...
public class CarSearchService {
    ...
    @Override
    public Page<CarSearchResult> simpleSearch(StringSearchCarDemoRequest request) {
...
### Using [`NaturalIdSearchExecturor`][natural-id-search-executor-url]
...
## [SearchConfiguration][search-configuration-url]
...
Supported joins are **inner joins** and **left outer joins**.
...
[`SearchConfiguration`][search-configuration-url] supports matching **all** values (default behaviour), operator and is used for query predicate creation.
[`SearchConfiguration`][search-configuration-url] also supports matching **any** value by setting `anyMatch` parameter to `true`, then or operator is used when creating query predicate.
