 ...
 * @param encryptPassword             Optional parameter. If it is null data is encrypted with a randomly generated password on each application restart. To persist encrypted data
 *                                    across restarts this parameter must be specified together with encryptSalt (configuring only one of the two is rejected at startup).
 * @param encryptSalt                 Optional parameter. Must be a hex-encoded string (for example a value generated by {@code KeyGenerators.string().generateKey()}). If it is null data is
 *                                    encrypted with a randomly generated salt on each application restart. To persist encrypted data across restarts this parameter must be specified
 *                                    together with encryptPassword (configuring only one of the two is rejected at startup).
 */
