...
It is built on top of `spring-data-jpa` module and adds custom repository interfaces whose implementation provides searching functionality.
...
- [`SearchExecutor<T>`][search-executor-url]
- [`StringSearchExecutor<T>`][string-search-executor-url]
- [`NaturalIdSearchExecutor<T>`][natural-id-search-executor-url]

[`SearchExecutor<T>`][search-executor-url] interface enables searching by properties defined in a class and [`StringSearchExecutor<T>`][string-search-executor-url] interface enables searching by
a string and a provided list of properties to search. When using Hibernate as JPA implementation [`NaturalIdSearchExecutor<T>`][natural-id-search-executor-url] is also available that enables
searching by `@NaturalId` annotated properties.
