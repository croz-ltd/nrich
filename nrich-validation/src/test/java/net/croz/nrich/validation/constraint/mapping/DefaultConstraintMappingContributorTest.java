...
class DefaultConstraintMappingContributorTest {

    @Mock
    private ConstraintMappingContributor.ConstraintMappingBuilder constraintMappingBuilder;

    @InjectMocks
    private DefaultConstraintMappingContributor defaultConstraintMappingContributor;
    ...
    void shouldRegisterConstraints() {
        ...
        doReturn(constraintMapping).when(constraintMappingBuilder).addConstraintMapping();

        // when
        defaultConstraintMappingContributor.createConstraintMappings(constraintMappingBuilder);

        // then
        verify(constraintMapping.constraintDefinition(ValidOib.class)).validatedBy(ValidOibValidator.class);
        verify(constraintMapping.constraintDefinition(ValidSearchProperties.class)).validatedBy(ValidSearchPropertiesValidator.class);
        verify(constraintMapping.constraintDefinition(ValidRange.class)).validatedBy(ValidRangeValidator.class);
        verify(constraintMapping.constraintDefinition(MaxSizeInBytes.class)).validatedBy(MaxSizeInBytesValidator.class);
        verify(constraintMapping.constraintDefinition(NotNullWhen.class)).validatedBy(NotNullWhenValidator.class);
        verify(constraintMapping.constraintDefinition(NullWhen.class)).validatedBy(NullWhenValidator.class);
        verify(constraintMapping.constraintDefinition(ValidFile.class)).validatedBy(ValidFileValidator.class);
        verify(constraintMapping.constraintDefinition(ValidFileResolvable.class)).validatedBy(ValidFileResolvableValidator.class);
        verify(constraintMapping.constraintDefinition(InList.class)).validatedBy(InListValidator.class);
        verify(constraintMapping.constraintDefinition(SpelExpression.class)).validatedBy(SpelExpressionValidator.class);
    }
}
