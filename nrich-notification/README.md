...
|:---------------------:|-------------------------------------------------------------------------------------------------|
|        `INFO`         | information that an action was successfully executed such as 'Entity has been saved' or similar |
|       `WARNING`       | information that some validation has failed or similar                                          |
|        `ERROR`        | information that an exception occurred while processing a request                               |
...
### [ConstraintConversionService][constraint-conversion-service-url]

[`ConstraintConversionService`][constraint-conversion-service-url] is a service responsible for converting `javax.validation.ConstraintViolation` list to Spring `Errors` instance.
...
### [NotificationMessageResolverService][nrich-notification-message-resolver-service-url]

[`NotificationMessageResolverService`][nrich-notification-message-resolver-service-url] is a service responsible for resolving messages from message codes and `ObjectErrors`.
...
### [NotificationResolverService][notification-resolver-service-url]
...
### [NotificationResponseService][notification-response-service-url]

[`NotificationResponseService`][notification-response-service-url] is a service that adds support for additional data in notifications by introducing additional methods that
return [`NotificationDataResponse`][notification-data-response-url].
When using Spring Web MVC users can use [`NotificationResponseService`][notification-response-service-url] implementation
[`WebMvcNotificationResponseService`][web-mvc-notification-response-service-url] that is a wrapper around [`NotificationResolverService`][notification-resolver-service-url] providing a convenient way
of returning a notification with a response.

---

### Default `MessageSource` setup

`nrich-notification` also includes [`nrich-notification-messages`][nrich-notification-messages-url] property files that contain required default messages and should be added to
...
[`NotificationResponseService`][notification-response-service-url] creates notifications that are then sent to the client. Such and advice is already implemented in [`nrich-webmvc`][nrich-webmvc-url].
...
### [AdditionalNotificationData][additional-notification-data-url]
